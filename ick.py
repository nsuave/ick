...
def findLatest(domain, series):
    ...
    return matchingURI, latestChapter

###
### GET ALL URL'S VIA LATEST CHAPTER PAGE
###

def findAllUrl(latestChapter, matchingURI):
...
def downloadImages(latestChapter, series, imageURLS):
...
def emailPdf(mailUrl, mailKey, mailSandbox, mailRecipient, convertedFile):
    ...
    mailStatusCode = mailRequest.status_code
    mailStatusText = mailRequest.text
    return mailStatusCode, mailStatusText

##
## Execution
##
matchingURI, latestChapter = findLatest(domain, series)
chapterURLList = findAllUrl(latestChapter, matchingURI)
imageURLS = findUrl(chapterURLList)
directory = downloadImages(latestChapter, series, imageURLS)
convertedFile = createPdf(directory)
mailStatusCode, mailStatusText = emailPdf(mailUrl, mailKey, mailSandbox, mailRecipient, convertedFile)
print(mailStatusCode)
print(mailStatusText)
